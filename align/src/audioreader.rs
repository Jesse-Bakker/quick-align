...
fn convert_and_push_frame(frame: &Audio, data: &mut Vec<f32>) {
    ...
    data.extend_from_slice(unsafe { slice::from_raw_parts(ptr as *const f32, frame.samples()) });
}
fn receive_and_process_decoded_frames(
    ...
    while decoder.receive_frame(&mut decoded).is_ok() {
        ...
        convert_and_push_frame(&new_frame, data);
    }
...
pub(crate) fn read_and_transcode_file(
    ...
    for (stream, packet) in ictx.packets() {
        if stream.index() == input_index {
            ...
            receive_and_process_decoded_frames(&mut decoder, &mut resampler, &mut data)?;
        }
    }
    decoder.send_eof()?;
    receive_and_process_decoded_frames(&mut decoder, &mut resampler, &mut data)?;
